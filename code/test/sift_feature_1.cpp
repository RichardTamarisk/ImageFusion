...
{
    // 读取输入图像
    cv::Mat image = cv::imread("l.png");

    // 计算尺度空间
    double sigma_min = 1.6, sigma_max = 6.4;
    int num_scales = 5;
    ...
    // 移除不稳定的边缘响应点
    double r = 10.0;
    std::vector<cv::Point3i> stable_extrema = removeEdgeResponse(dog, extrema, r);

    // 在输入图像上绘制稳定的特征点
    cv::Mat output_image = image.clone();
    for(size_t i = 0; i < stable_extrema.size(); i++)
    {
        cv::Point3i p = stable_extrema[i];
        cv::circle(output_image, cv::Point(p.z, p.y), 3, cv::Scalar(0, 0, 255), 2);
    }

    // 存储图像
    imwrite("hessian_feature_3_result.png", output_image);
